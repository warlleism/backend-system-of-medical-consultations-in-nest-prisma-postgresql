    ...
    async create(@Body() patient: IPatient) {
        try {
            ...
            throw new HttpException({
                ...
                error: error.message.includes('Unique constraint failed on the fields: (`cpf`)') ? 'Patient already exists' : error.message,
    ...
    async delete(@Param('id') id: string) {
        try {
            ...
            throw new HttpException({
                statusCode: HttpStatus.BAD_REQUEST,
                message: 'Patient creation failed',
                error: error.message.includes('Record to delete does not exist') ? 'Patient not found' : error.message,
            }, HttpStatus.BAD_REQUEST);
        }
    }
